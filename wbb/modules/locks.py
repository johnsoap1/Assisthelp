...
@app.on_message(filters.command(["lock", "unlock"]) & ~filters.chat(ChatType.PRIVATE))
...
@app.on_message(filters.command("locks") & ~filters.chat(ChatType.PRIVATE))
@capture_err
...
@app.on_message(filters.text & ~filters.chat(ChatType.PRIVATE), group=69)
async def url_detector(_, message):
    ...
    if user.id in mods or user.id in SUDOERS_SET:
        return
